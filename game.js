...
// === Keyboard focus guard: keep arrow-key control inside the canvas only ===
(function() {
  // Make canvas the active element for keyboard control
  try { if (canvas && typeof canvas.focus === 'function') canvas.focus(); } catch(e){}

  // Remove keyboard focus from header UI (still clickable by mouse)
  [controlMode, btnRestart, btnStart, btnPause].forEach(el => {
    if (el) el.setAttribute('tabindex', '-1');
  });

  // Helper: when arrow keys press anywhere, force focus back to canvas
  function isArrow(code){ return code==='ArrowLeft'||code==='ArrowRight'||code==='ArrowUp'||code==='ArrowDown'; }

  window.addEventListener('keydown', (ev) => {
    if (isArrow(ev.code)) {
      // Prevent default so selects / page scroll won't react
      ev.preventDefault();
      // If focus drifted to any control, bring it back to canvas
  ...
  // Also if user clicks on the page, re-focus canvas when using keyboard mode
...
function pickOp() {
 ...
 // x 제거
  ...
  function updateCurrentScoreUI(){ if (currentScoreEl) currentScoreUIImpl(); }
  function currentScoreUIImpl(){ currentScoreEl.textContent = `현재 점수: ${Math.floor(score)}`; }
// Waves
  let wave = 1;
  let inWave = true;
  let restTimer = 0;
  ...
  let waveKills = 0;
  let score = 0;
  let highScore = Number(localStorage.getItem('mf_highscore') || 0) || 0;
  ...
  function resetGame() {
    score = 0; // reset run score each game
    if (currentScoreEl) { currentScoreEl.textContent = `현재 점수: 0`; }
  ...
  window.addEventListener('keydown', (e)=>{
    // Space: pause/resume
    if (e.code === 'Space') { e.preventDefault(); paused = !paused; if (btnPause) btnPause.textContent = paused ? '계속' : '일시정지'; updateCurrentScoreUI(); return; }
  ...
  function update(dt) {
    ...
    if (controlMode.value === 'keyboard') {
      ...
      if (keys.up) dirY -= 1;
      if (keys.down) dirY += 1;
      // 기본 이동 속도(픽셀/초) - 안정적인 속도 유지
      const SPEED = 220;
      player.speedX = dirX * SPEED;
      player.speedY = dirY * SPEED;
      // 프레임레이트에 독립적인 이동 (dt 적용)
      player.x = clamp(player.x + player.speedX * dt, minX, maxX);
      player.y = clamp(player.y + player.speedY * dt, minY, maxY);
      player.targetX = player.x; player.targetY = player.y;
    } else {
      // 마우스 추적 속도 약간 완화
      const lerp = 0.12;
      player.x += (player.targetX - player.x) * lerp;
      player.y += 1.5 *  (player.targetY - player.y) * lerp;
...
// camera shake transform
  ...
  btnRestart.addEventListener('click', () => { if (score > highScore) { highScore = Math.floor(score); localStorage.setItem('mf_highscore', String(highScore)); }
    ...
    updateCurrentScoreUI(); });
  btnPause.addEventListener('click', () => { paused = !paused; if (score > highScore) { highScore = Math.floor(score); localStorage.setItem('mf_highscore', String(highScore)); updateHiscoreUI();
    updateCurrentScoreUI(); }
    updateCurrentScoreUI(); if (btnPause) btnPause.textContent = paused ? '계속' : '일시정지'; updateCurrentScoreUI(); });
